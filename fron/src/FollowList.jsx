...
class FollowList extends Component {
  ...
  getProfileData = async (v) => {
    try {
      let url = ''
      url = 'https://101.200.52.246:8080/api/follow_relations' + '?format=json' + '&page=' + this.page + '&page_size=' + count + '&user=' + window.localStorage.getItem('user_id')
  ...
  handleFollowList = async (page) => {
    ...
    try {
      let url = ''
      url = 'https://101.200.52.246:8080/api/follow_relations' + '?format=json' + '&page=' + page + '&page_size=' + count + '&user=' + window.localStorage.getItem('user_id')
      const response = await axios.get(url)
